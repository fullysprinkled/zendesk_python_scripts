...
def parse_links(link_page):
    ...
    while len(link_page) > 0: #loop through link list of dicts, paginate while > 0 links
        ...
        response = session.get("https://{YOUR_ORG}.zendesk.com/api/services/jira/links?since_id="+str(link_page[-1]["id"]))
...
def chunk_ids_to_api_result(id_list):
    ...
    show_many_url = "https://{YOUR_ORG}.zendesk.com/api/v2/tickets/show_many?ids="
    all_ticket_data= []
    chunk_count=1
    for chunk in chunks:
...
def main():
    ...
    url = "https://{YOUR_ORG}.zendesk.com/api/services/jira/links?since_id="+str(since_id)
